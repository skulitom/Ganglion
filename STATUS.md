...
- **The model against the reference, live.** Time to the first shot, median over twenty trials each:
  ...
  v1b stays slower. The v6 readout (the goal as a direction) fires 64 of 92 (70%) at 1.07 s with the
  lowest tracking error of any condition (121 px against the reference's 159, p 0.005).
  The model does not make the chain faster than the reference live, and no longer slower; it now
  tracks tighter. Next: a readout that stops on its own, so the envelope is not the brake.
  [Details](docs/bench/HALFLIFE.md).
- **Thirty trials each, one grunt, no stray controller.** Every earlier live trial ran with a virtual
  controller holding move-backward (Half-Life reads pads); comparisons stand, absolute figures do not
...
- **Encounters at range.** Every point-blank trial ran at the easy skill with the game's autoaim on. A new
  harness stages saves at real encounters (hard skill, no autoaim), runs key-only trials in seeded pairs and
  reads damage and kills from the save file. Two scenes are staged (three grunts on a ledge at 700 units; a
  patrol at 520 to 1,260 units). Reference-only probes: on the ledge, damage 0 of 240 in nine trials of ten
  (a hit kicks the view and the template slides onto scenery); in the lobby the motion watch does not see
  grunts walking along the line of sight. The bottleneck at range is perception, not the controller; no
  model trial was run. [Details](docs/bench/HALFLIFE.md).
- **The seat's viewer no longer moves the user's pointer**: with the seat tool's fix in, a game launch, map
  loads and console toggles gave no teleport of the main pointer in 100 s of sampling.
...
- The suite passes **182 tests** (four optional checks skipped). Application-specific
