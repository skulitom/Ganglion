...
def fit_ego_motion(flow, iterations=3, stride=FIT_STRIDE):
    ...
    for _ in range(iterations):
        ...
        weights = sampled <= max(0.5, 2.5 * float(np.median(sampled)))
...
def wide_field(model, scale, lag_seconds):
...
def detect_flow(frame: np.ndarray, spec, state: dict, *, captured: float, moving: bool):
    ...
    model = fit_ego_motion(flow)
    ego = wide_field(model, scale, captured - reference_time)
