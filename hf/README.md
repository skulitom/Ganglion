...
- Under supervision, v6 does the reference controller's job in a live Half-Life fight, no
  better and not measurably worse. With both sides' steps limited in length to the same
  1,200 px/s (42 model trials against 45 of the reference, one grunt, alternating blocks in two
  sessions): first shot 0.94 against 1.14 s at the median (p 0.28), fired on 61% against 68%
  of align intents (p 0.08 on per-trial shares), tracking error 152 against 161 px (p 0.70).
  An earlier result here (fewer intents fired, 58% against 75%) was withdrawn: repeated, it
  reversed, the reference alone moved from 75% to 60% between sessions, and the reference had
  been allowed up to 41% more speed than the model by a per-axis step clamp.
